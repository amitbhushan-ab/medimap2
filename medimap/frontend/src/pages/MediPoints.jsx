...
const TIERS = [
  { tier: 1, points: 500,  discount: 20, code: 'MEDI20', icon: '🥉', color: 'from-amber-400 to-orange-500',  descEn: '20% OFF on medicines', descHi: 'दवाइयों पर 20% छूट' },
  { tier: 2, points: 1000, discount: 30, code: 'MEDI30', icon: '🥈', color: 'from-gray-400 to-gray-600',     descEn: '30% OFF on medicines', descHi: 'दवाइयों पर 30% छूट' },
  { tier: 3, points: 2000, discount: 50, code: 'MEDI50', icon: '🥇', color: 'from-yellow-400 to-amber-500',  descEn: '50% OFF — Premium Reward', descHi: '50% छूट — प्रीमियम इनाम' },
];
...
export default function MediPoints() {
  ...
  async function redeem(tier) {
    ...
    try {
      ...
      showToast(lang === 'hi' ? `🎉 कूपन ${d.coupon.code} उत्पन्न हुआ!` : `🎉 Coupon ${d.coupon.code} generated!`);
  ...
  if (!user) return (
    <div className="max-w-lg mx-auto px-4 py-16 text-center">
      ...
      <p className="text-gray-500 mb-6">{lang === 'hi' ? 'मेडीपॉइंट्स कमाने और भुनाने के लिए लॉग इन करें!' : 'Login to earn and redeem MediPoints!'}</p>
      <Link to="/login" className="btn-primary">{lang === 'hi' ? 'जारी रखने के लिए लॉग इन करें' : 'Login to Continue'}</Link>
    </div>
  );
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">{lang === 'hi' ? 'पॉइंट्स कमाएं, दवाइयों पर पैसे बचाएं' : 'Earn points, save money on medicines'}</p>
      </div>

      {/* Points Card */}
      <div className="rounded-2xl p-6 mb-6 text-white text-center" style={{ background: 'linear-gradient(135deg,#2E7DFF,#00C2A8)' }}>
        <p className="text-white/70 text-sm mb-1">{lang === 'hi' ? 'आपका बैलेंस' : 'Your Balance'}</p>
        ...
        {nextTier && (
          <div className="mt-4">
            ...
            <p className="text-white/70 text-xs mt-2">{lang === 'hi' ? `अगले इनाम के लिए ${pointsToNext} और पॉइंट्स` : `${pointsToNext} more points for next reward`}</p>
          </div>
        )}
      </div>
      ...
      <div className="card p-5 mb-6">
        <h2 className="font-bold text-gray-900 mb-3">💡 {lang === 'hi' ? 'पॉइंट्स कैसे कमाएं' : 'How to Earn Points'}</h2>
        <div className="space-y-3">
          {[
            { icon: '📸', textEn: 'Submit medicine price update', textHi: 'दवा की कीमत अपडेट जमा करें', points: '+20 pts', descEn: 'When your submission is approved by admin', descHi: 'जब आपका अपडेट व्यवस्थापक द्वारा स्वीकृत किया जाता है' },
            { icon: '⭐', textEn: 'Rate a pharmacy', textHi: 'फार्मेसी को रेट करें', points: '+5 pts', descEn: 'After leaving a review', descHi: 'समीक्षा छोड़ने के बाद' },
            { icon: '👤', textEn: 'Complete your profile', textHi: 'अपनी प्रोफ़ाइल पूरी करें', points: '+10 pts', descEn: 'One-time bonus', descHi: 'एकमुश्त बोनस' },
          ].map((item, i) => (
            <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
              <span className="text-xl">{item.icon}</span>
              <div className="flex-1"><p className="text-sm font-semibold text-gray-900">{lang === 'hi' ? item.textHi : item.textEn}</p><p className="text-xs text-gray-400">{lang === 'hi' ? item.descHi : item.descEn}</p></div>
              <span className="text-sm font-bold text-emerald-600 whitespace-nowrap">{item.points}</span>
            </div>
          ))}
        </div>
        <div className="mt-3 text-center">
          <Link to="/submit-price" className="btn-primary text-sm !py-2">📸 {lang === 'hi' ? 'कीमत जमा करें और पॉइंट कमाएं' : 'Submit Price & Earn Points'}</Link>
        </div>
      </div>

      {/* Redeem Rewards */}
      <div className="card p-5 mb-6">
        <h2 className="font-bold text-gray-900 mb-4">🎁 {lang === 'hi' ? 'इनाम भुनाएं' : 'Redeem Rewards'}</h2>
        <div className="space-y-3">
          {TIERS.map(tier => {
              ...
              <div key={tier.tier} className={`rounded-2xl p-4 border-2 transition-all ${canRedeem ? 'border-[#2E7DFF] bg-blue-50' : 'border-gray-100'}`}>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-gray-900">{tier.discount}% {lang === 'hi' ? 'छूट कूपन' : 'OFF Coupon'}</p>
                      <p className="text-xs text-gray-500">{lang === 'hi' ? tier.descHi : tier.descEn}</p>
                      <p className="text-xs font-semibold text-[#2E7DFF] mt-0.5">{tier.points} {lang === 'hi' ? 'पॉइंट्स आवश्यक' : 'points required'}</p>
                    </div>
                  </div>
                  <button
                  ...
                  >
                    {redeeming === tier.tier ? '...' : canRedeem ? (lang === 'hi' ? 'भुनाएं' : 'Redeem') : (lang === 'hi' ? `${tier.points - data.points} और चाहिए` : `Need ${tier.points - data.points} more`)}
                  </button>
                </div>
      ...
      {data.coupons?.length > 0 && (
        <div className="card p-5 mb-6">
          <h2 className="font-bold text-gray-900 mb-3">🎟️ {lang === 'hi' ? 'मेरे कूपन' : 'My Coupons'}</h2>
            ...
            {data.coupons.map(c => (
              <div key={c.id} className={`rounded-xl p-4 border-2 ${c.isUsed ? 'border-gray-100 bg-gray-50 opacity-60' : 'border-dashed border-[#2E7DFF] bg-blue-50'}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-bold text-gray-900">{c.discount}% {lang === 'hi' ? 'छूट' : 'OFF'}</p>
                    <p className="text-xs text-gray-400">{lang === 'hi' ? 'समाप्ति:' : 'Expires:'} {new Date(c.expiresAt).toLocaleDateString('en-IN')}</p>
                  </div>
                  <div className="text-right">
                    ...
                    {c.isUsed && <p className="text-xs text-gray-400 mt-1">{lang === 'hi' ? 'इस्तेमाल किया गया' : 'Used'}</p>}
                  </div>
                </div>
              </div>
            ))}
      ...
      {data.transactions?.length > 0 && (
        <div className="card p-5">
          <h2 className="font-bold text-gray-900 mb-3">📋 {lang === 'hi' ? 'इतिहास' : 'History'}</h2>
          <div className="space-y-2">
            {data.transactions.slice(0, 10).map(tx => (
              <div key={tx.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                ...
                <span className={`font-bold text-sm ${tx.points > 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                  {tx.points > 0 ? '+' : ''}{tx.points} {lang === 'hi' ? 'पॉइंट्स' : 'pts'}
                </span>
              </div>
            ))}
          </div>
